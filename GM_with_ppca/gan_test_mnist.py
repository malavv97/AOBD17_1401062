...
def generator(z, batch_size, z_dim, W, t_mean,reuse=False):
    ...
    final = tf.zeros([0,28,28,1])
    for i in range(batch_size):
    	c = tf.reshape(tf.add(tf.matmul(W,Result[i]),t_mean),[1,28,28,1])
...
x_train = read('generated_pca/'+str(no_train_data)+'_'+str(train_data)+'_data.file')
W = read('generated_pca/'+str(no_train_data)+'_'+str(train_data)+'_W.file')
t_mean = read('generated_pca/'+str(no_train_data)+'_'+str(train_data)+'_t_mean.file')
...
sess = tf.Session()
x_placeholder = tf.placeholder(tf.float32, shape = [None,28,28,1]) #Placeholder for input images to the discriminator
...
for i in range(iterations):
    ...
    for j in range(batch_size):
    	real_image_batch[j,:,:,:] = np.reshape(x_train[randint(0,no_train_data-1),:],[1,28,28,1])
...
plt.title('Discriminator loss in proposed GM - MNIST Dataset')
plt.show()

write(Dloss,'Results/Dloss_1000_proposed_gm.file')
